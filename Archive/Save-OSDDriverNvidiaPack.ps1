<#
.SYNOPSIS
Downloads and creates Nvidia Video Driver Packs
...
.LINK
https://osddrivers.osdeploy.com/module/functions/save-osddrivernvidiapack
#>
function Save-OSDDriverNvidiaPack {
    [CmdletBinding(DefaultParameterSetName = 'Default')]
    Param (
        #Fully automatic Pack creation
        [Parameter(ParameterSetName = 'QuickPack', Mandatory = $true)]
        [ValidateSet('NvidiaPack 10.0 x64','NvidiaPack 10.0 x86','NvidiaPack 6.1 x64','NvidiaPack 6.1 x86')]
        [string]$QuickPack,
    ...
    Begin {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'QuickPack') {
            ...
            $Pack = $true
            $SkipGridView = $true
            if ($QuickPack -eq 'NvidiaPack 10.0 x64') {
                $OsArch = 'x64'
                $OsVersion = '10.0'
            }
            if ($QuickPack -eq 'NvidiaPack 10.0 x86') {
                $OsArch = 'x86'
                $OsVersion = '10.0'
            }
            if ($QuickPack -eq 'NvidiaPack 6.1 x64') {
                $OsArch = 'x64'
                $OsVersion = '6.1'
            }
            if ($QuickPack -eq 'NvidiaPack 6.1 x86') {
                $OsArch = 'x86'
                $OsVersion = '6.1'
            }
        } else {

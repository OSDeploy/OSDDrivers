...
function New-OSDDriverCabFileCopy
{
    [CmdletBinding()]
    Param (
    ...
    Process {
        ...
        if (Test-Path "$DriverExpandPath\AutoDriver.adpnp") {
            Copy-Item -Path "$DriverExpandPath\AutoDriver.adpnp" -Destination "$DestinationDirectory\$SourceName.adpnp"
        }
